...
class CallSAConfig(BasePage):
    CALL_CONFIG_SCOPE = "//div[@id='media-configuration-call']"


    TAB_SENTIMENT_ANALYSIS = (By.XPATH, f"{CALL_CONFIG_SCOPE}//button[@id='sa-tab-call']")

    TOGGLE_SA = (By.XPATH,
                 f"{CALL_CONFIG_SCOPE}//*[@id='sa-tab-call-pane']/div/form/div/div/div[2]/div/div[1]/div/label[2]/span")
    SA_TOGGLE_INPUT = (By.XPATH, "//input[@name='isEnableSA']")


    SA_SAVE_BTN = (By.XPATH, f"{CALL_CONFIG_SCOPE}//*[@id='sa-tab-call-pane']//button[@title='Save']")
    SA_RESET_BTN = (
        By.XPATH,
        "//div[@id='sa-tab-call-pane']//button[@title='Reset']"
    )
    SA_EDIT_BTN = (By.XPATH,
                   f"{CALL_CONFIG_SCOPE}//*[@id='sa-tab-call-pane']/div/form/div/div/div[2]/div/div[2]/div/div/button")
    SA_TEST_EDIT_BTN = (By.XPATH, f"{CALL_CONFIG_SCOPE}//*[@id='sa-tab-call-pane']//button[text()='Test']/following-sibling::button[@title='Edit']")
    SA_EDIT_KEY_FILE = (By.XPATH, f"{CALL_CONFIG_SCOPE}//*[@id='sa-tab-call-pane']//input[@name='prevKeyFile']/following-sibling::button[@title='Edit']")
    SA_SELECT_ENGINE = (By.XPATH, f"{CALL_CONFIG_SCOPE}//select[@name='engineNameSA']")
    SA_RADIO_CONVERSION_MODE = (By.XPATH, f"{CALL_CONFIG_SCOPE}//*[@id='manualModeSa_call']")
    ...
    SA_RADIO_CONVERSION_MODE_MANUAL = (By.XPATH, f"{CALL_CONFIG_SCOPE}//*[@id='manualModeSa_call']")
    SA_RADIO_CONVERSION_MODE_AUTO = (By.XPATH, f"{CALL_CONFIG_SCOPE}//*[@id='autoModeSa_call']")
    ...
    EDIT_LOC_MAIN = (By.XPATH, "//div[@id='sa-tab-call-pane']//button[@title='Edit']")
    EDIT_LOC_TEST = (By.XPATH,
                     "//div[@id='sa-tab-call-pane']//button[text()='Test']/following-sibling::button[@title='Edit']")
    EDIT_LOC_TEST_AGAIN = (By.XPATH,
                           "//div[@id='sa-tab-call-pane']//button[text()='Test Again']/following-sibling::button[@title='Edit']")

    BTN_EDIT_MAIN = (By.XPATH,
                     "//div[@id='sa-tab-call-pane']//button[@class='section_form_acts_btn' and @title='Edit']")
    ...
    def reconfig_sa(self):
        try:
            ...
            logger.info(
                f"SA DATA: {self.sentimentAnalysis}, {self.keyFile}, {self.engine}, {self.mode}, {self.sentiment_text}"
            )

            # MUST WAIT BEFORE ACCESSING ANY FORM FIELDS
            form_loaded = self.wait_for_sa_form()

            if not form_loaded:
                logger.warning("Full SA form unavailable — Performing minimal SA configuration only.")
                self.toggle_sa()
                self.click_save()
                return

            self.loader.load()
            logger.info(f"SA form is ready for input.")
            self.toggle_sa()
            self.select_engine()
            self.choose_conversion_mode()

            logger.info("Going to re-upload key file")
            self.key_file_edit()
            self.key_file_input()
    ...
    def tab_sa(self):
        tab = self.wait_until_clickable(self.TAB_SENTIMENT_ANALYSIS)
        tab.click()
        self.loader.load()
